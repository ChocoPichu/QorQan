...
async def start_handler(message: Message, state: FSMContext):
    ...
    # FIX: If first time (no lang set), show language picker first
    if not lang:
    ...
    # Already has a language — clear FSM state but keep lang
...
async def support_handler(message: Message, state: FSMContext):
    ...
    if online_count == 0:
        ...
        # FIX: Don't return here — still let them queue, just warn them first
...
async def cancel_waiting_callback(callback: CallbackQuery, state: FSMContext):
    ...
    # FIX: Compare against .state string, not the State object itself
...
# NOTE: This handler must stay LAST — it's a catch-all for non-command text.
# The specific menu handlers above take priority because they're registered first.
...
async def ingest_kid_message(message: Message, state: FSMContext):
    ...
    if not active_session:
        # FIX: Was silently returning — now gives a gentle nudge
